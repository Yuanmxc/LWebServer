...
void FastCgi::Conection() {
    ...
    int ret = ::connect(socket_.fd(), (struct sockaddr*)&ServerAddress,
...
FCGI_Header FastCgi::CreateHeader(int type, int request, int contentLength,
    ...
    header.contentLengthB1 =
        (unsigned char)((contentLength >> 8) & 0xff);  // 高八位
    header.contentLengthB0 = (unsigned char)(contentLength & 0xff);  // 低八位

    header.paddingLength =
        (unsigned char)paddingLength;  // 填充字节的长度 保证字节对齐 TODO 查下
    header.reserved = 0;  // 保留字节赋为0
    return header;        // RVO
}

FCGI_BeginRequestBody FastCgi::CreateBeginRequestBody(int role,
                                                      int keepConnection) {
    FCGI_BeginRequestBody body;

    body.roleB1 = (unsigned char)((role >> 8) &
                                  0xff);  // 两个字节表示期望php-fpm扮演的角色
                                          // 一般为响应者
...
bool FastCgi::StartRequest() {
    FCGI_BeginRequestRecord FirstRecord;

    FirstRecord.header =
        CreateHeader(FCGI_BEGIN_REQUEST, requestId_, sizeof FirstRecord.body);
    FirstRecord.body = CreateBeginRequestBody(
        FCGI_RESPONDER, 0);  // 当做响应器 发出消息执行后返回
...
void FastCgi::CreateContentValue(const std::string& name,
                                 int nameLen,  // ContentBuffPtr为要写入的地方
                                 const std::string& value, int valueLen,
                                 unsigned char* ContentBuffPtr,
                                 int* ContentLen) {
    ...
    if (nameLen < 128) {  // 如果nameLen长度小于128字节
        ...
        *ContentBuffPtr++ =
            (unsigned char)((nameLen >> 24) | 0x80);  // 感觉后面并没有什么意义
    ...
    std::for_each(name.cbegin(), name.cend(),
                  [&](char ch) { *ContentBuffPtr++ = ch; });
...
void FastCgi::SendContent(const std::string& tag, const std::string& value) {
    ...
    CreateContentValue(tag, tag.size(), value, value.size(), bodyBuffer,
                       &ContentLength);  // 得到发送的内容与长度

    FCGI_Header ContentHeader(
        CreateHeader(FCGI_PARAMS, requestId_, ContentLength));
...
void FastCgi::SendRequest(const std::string& data, size_t len) {
    FCGI_Header IntervalHead(
...
std::string FastCgi::ReadContent() {
    ...
    while (::read(socket_.fd(), &parsering, FCGI_HEADER_LEN) > 0) {
        if (parsering.type == FCGI_STDOUT) {  // 正常输出
            ContentLength = (parsering.contentLengthB1 << 8) +
                            (parsering.contentLengthB0);  // 得到内容长度
            ...
            ContentLength =
                (parsering.contentLengthB1 << 8) + (parsering.contentLengthB0);
            memset(Content, 0, CONTENT_BUFFER_LEN);
...
void FastCgi::GetContent(char* data) {

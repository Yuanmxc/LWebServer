...
class Threadsafe_unordered_map {
   private:
    class buket_type {
       ...
       private:
        ...
        buket_iterator find_entry(const Key& key) {  // TODO const有点意思
                                                     // 再看看
            return std::find_if(
                data.begin(), data.end(),
                [&](const buket_value& para) { return para.first == key; });
        }
       ...
       public:
        Value value_of(const Key& key, const Value& default_value) const {
            ...
            return (value_iterator == data.end()) ? default_value
   ...
   public:
    ...
    explicit Threadsafe_unordered_map(unsigned num_bukets = 23,
    ...
    Threadsafe_unordered_map& operator=(const Threadsafe_unordered_map&) =
        delete;

    value_type value_of(const key_type& key,
                        const value_type& value = value_type()) const {
    ...
    void remove(const key_type& key) { return get_buket(key).remove(key); }

    std::map<Key, Value> get_standard_map() const;
};
...
Threadsafe_unordered_map<Key, Value, Hash>::get_standard_map() const {
    ...
    for (size_t i = 0; i < bukets.size(); i++) {
        lk.emplace_back(
            std::unique_lock<boost::shared_mutex>(bukets[i]->mutex));
    }
    for (size_t i = 0; i < bukets.size(); i++) {

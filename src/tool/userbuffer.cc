...
void __attribute__((access(write_only, 1)))
UserBuffer::Read(char* Start, int bytes) {
...
char __attribute__((hot)) __attribute__((pure))
UserBuffer::Peek(int jump) const {
    if (Write_Spot < Read_Spot + jump && Read_Spot + jump < 0) {
        throw std::out_of_range(
            "'UserBuffer::Peek' The parameter is not in the vaild range.");
    }
...
int __attribute__((access(read_only, 1)))
UserBuffer::Write(char* Buf, int bytes) {
    if (bytes > Writeable()) {
        Move_Buffer();
    }
    if (bytes > Writeable()) {
...
int __attribute__((access(read_only, 1)))
UserBuffer::Write(const char* Buf, int bytes) {
    if (bytes > Writeable()) {
        Move_Buffer();
    }
    if (bytes > Writeable()) {
...
int __attribute__((access(read_only, 1)))
UserBuffer::SWrite(const char* format, va_list para) {

        ...
        while (true) {
            ...
            */
            ...
            } else  // 前面的操作都是在确保全局尾指针在全局头指针之后，只有这样才能安全的删除数据
            {
                ...
                // 在取出数据后，如果head指针没有落后，移动 old_head
                // 指针成功则退出
                if (::__sync_bool_compare_and_swap(&head_, old_head,
                                                   first_node)) {
                    break;
                }
            }
        ...
        while (true) {
            ...
            if (old_tail_next == nullptr) {
                if (::__sync_bool_compare_and_swap(
                        &(old_tail->next_), old_tail_next, enqueue_node)) {
                    break;
                }
            } else {
                ...
                ::__sync_bool_compare_and_swap(&(tail_), old_tail,
                                               old_tail_next);
                continue;
            }
        }
